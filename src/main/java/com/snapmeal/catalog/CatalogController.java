...
public class CatalogController {
    private static final String DISH_CACHE_PREFIX = "snap-meal:catalog:dish:";
    private final JdbcTemplate jdbc; private final CatalogCache cache;
    public CatalogController(JdbcTemplate jdbc,CatalogCache cache){this.jdbc=jdbc;this.cache=cache;}
    ...
    @PostMapping("/admin/categories") public ApiResponse<Void> addCategory(@RequestBody CategoryRequest r){jdbc.update("insert into category(type,name,sort,status) values(?,?,?,?)",r.type==null?1:r.type,r.name,r.sort==null?nextCategorySort():r.sort,r.status==null?1:r.status);return ApiResponse.ok();}
    @PutMapping("/admin/categories/{id}") public ApiResponse<Void> editCategory(@PathVariable long id,@RequestBody CategoryRequest r){jdbc.update("update category set type=coalesce(?,type),name=?,sort=coalesce(?,sort),status=coalesce(?,status),update_time=current_timestamp where id=?",r.type,r.name,r.sort,r.status,id);return ApiResponse.ok();}
    ...
    public ApiResponse<List<Map<String,Object>>> dishes(@RequestParam(required=false) Long categoryId,@RequestParam(required=false) String name){String key=DISH_CACHE_PREFIX+(categoryId==null?"all":categoryId)+":"+(name==null||name.trim().isEmpty()?"all":name.trim());return ApiResponse.ok(cache.get(key,()->queryDishes(categoryId,name)));}
    private List<Map<String,Object>> queryDishes(Long categoryId,String name){StringBuilder sql=new StringBuilder("select d.*,c.name category_name from dish d join category c on c.id=d.category_id where 1=1");List<Object> args=new ArrayList<>();if(categoryId!=null){sql.append(" and d.category_id=?");args.add(categoryId);}if(name!=null&&!name.trim().isEmpty()){sql.append(" and d.name like ?");args.add("%"+name.trim()+"%");}sql.append(" order by d.id desc");return jdbc.queryForList(sql.toString(),args.toArray());}
    @PostMapping("/admin/dishes") public ApiResponse<Void> addDish(@RequestBody DishRequest r){jdbc.update("insert into dish(category_id,name,price,image,description,status) values(?,?,?,?,?,?)",r.categoryId,r.name,r.price,r.image,r.description,r.status);cache.evictByPrefix(DISH_CACHE_PREFIX);return ApiResponse.ok();}
    @PutMapping("/admin/dishes/{id}") public ApiResponse<Void> editDish(@PathVariable long id,@RequestBody DishRequest r){jdbc.update("update dish set category_id=?,name=?,price=?,image=?,description=?,status=?,update_time=current_timestamp where id=?",r.categoryId,r.name,r.price,r.image,r.description,r.status,id);cache.evictByPrefix(DISH_CACHE_PREFIX);return ApiResponse.ok();}
    @PatchMapping("/admin/dishes/{id}/status") public ApiResponse<Void> dishStatus(@PathVariable long id,@RequestParam int value){jdbc.update("update dish set status=?,update_time=current_timestamp where id=?",value,id);cache.evictByPrefix(DISH_CACHE_PREFIX);return ApiResponse.ok();}
    @DeleteMapping("/admin/dishes/{id}") public ApiResponse<Void> deleteDish(@PathVariable long id){Integer used=jdbc.queryForObject("select count(*) from setmeal_dish where dish_id=?",Integer.class,id);if(used!=null&&used>0)throw new BusinessException("菜品已关联套餐，不能删除");jdbc.update("delete from dish where id=?",id);cache.evictByPrefix(DISH_CACHE_PREFIX);return ApiResponse.ok();}
    ...
    private int nextCategorySort(){Integer max=jdbc.queryForObject("select coalesce(max(sort),0) from category where type=1",Integer.class);return (max==null?0:max)+10;}

    public static class CategoryRequest {public Integer type;public String name;public Integer sort;public Integer status;}
