...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    @Override public boolean preHandle(HttpServletRequest request,HttpServletResponse response,Object handler){if("OPTIONS".equalsIgnoreCase(request.getMethod())){return true;}boolean admin=request.getRequestURI().startsWith("/api/admin/");TokenService.Session s=tokens.verify(request.getHeader(admin?"token":"authentication"),admin?"ADMIN":"USER");request.setAttribute(SUBJECT_ID,s.getSubjectId());return true;}
}
